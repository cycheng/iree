...
TraversalResult Explorer::walkDefiningOps(Value value, ResultWalkFn fn) {
  ...
  // Move from std.call into all return points from the target.
...
TraversalResult Explorer::walkTransitiveUses(Value value, UseWalkFn fn) {
  ...
  auto traverseCallOp = [&](CallOpInterface callOp, unsigned operandIdx) {
    ...
    auto callableOp = symbolTables.lookupNearestSymbolFrom<CallableOpInterface>(
        callOp, targetSymbol);
    assert(callableOp);
    ...
    auto entryArg = callableOp.getCallableRegion()->getArgument(operandIdx);
    LLVM_DEBUG({
