...
Value ResourceStoreOp::getTiedResult(unsigned resultIndex) {
  return IREE::Util::TiedOpInterface::findTiedBaseValue(target());
}

::llvm::Optional<unsigned> ResourceStoreOp::getTiedResultOperandIndex(
    unsigned resultIndex) {
  return {0};  // target
}

SmallVector<int64_t, 4> ResourceStoreOp::getTiedResultOperandIndices() {
  return {0};  // target
}
...
void AsyncExecuteOp::build(OpBuilder &builder, OperationState &state,
                           ...
                           ValueRange awaitTimepoints, ValueRange operands,
  ...
  state.addOperands(awaitTimepoints);
  ...
  state.addAttribute("operand_segment_sizes",
                     builder.getI32VectorAttr({
                         ...
                         static_cast<int32_t>(awaitTimepoints.size()),
                     }));
  state.addRegion();
}
...
AsyncExecuteOp::cloneReplacementExcludingOperandsAndResults(
  ...
  SmallVector<Type, 4> newResultTypes = llvm::to_vector<4>(
      llvm::map_range(results(), [](auto value) { return value.getType(); }));
  SmallVector<Value, 4> newResultSizes = llvm::to_vector<4>(result_sizes());
  SmallVector<Value, 4> newOperandsValues = llvm::to_vector<4>(operands());
  SmallVector<Value, 4> newOperandSizes = llvm::to_vector<4>(operand_sizes());
  ...
  auto newOp = rewriter.create<AsyncExecuteOp>(
      getLoc(), newResultTypes, newResultSizes, await_timepoints(),
...
void AsyncWaveOp::build(OpBuilder &builder, OperationState &state,
                        TypeRange resultTypes, ValueRange resultSizes,
...
static LogicalResult verifyOp(AsyncWaveOp op) {
  if (failed(RegionBranchOpInterface::verifyTypes(op))) return failure();
  if (failed(verifyOpValueSizes(op, op.operands(), op.operand_sizes())) ||
      failed(verifyOpValueSizes(op, op.results(), op.result_sizes()))) {
    return failure();
...
OperandRange AsyncWaveOp::getSuccessorEntryOperands(unsigned index) {
  assert(index == 0 && "invalid region index");
  return operands();
}

void AsyncWaveOp::getSuccessorRegions(
    Optional<unsigned> index, ArrayRef<Attribute> operands,
    SmallVectorImpl<RegionSuccessor> &regions) {
  // Unconditional control flow into the region and back to the parent, so
  // return the correct RegionSuccessor purely based on the index being None or
...
Operation::operand_range AsyncWaveOp::getClosureOperands() {
  return operands();
}

Operation::result_range AsyncWaveOp::getClosureResults() { return results(); }

bool AsyncWaveOp::canClosureContainOp(Operation *op) { return false; }

IREE::Util::ValueAccess AsyncWaveOp::getOperandAccess(unsigned operandIndex) {
  auto arg = body().getArgument(operandIndex);
  return computeValueAccess(arg);
}

IREE::Util::ValueAccess AsyncWaveOp::getResultAccess(unsigned resultIndex) {
  auto yieldOp = cast<YieldOp>(body().getBlocks().front().getTerminator());
  return computeValueAccess(yieldOp.getOperand(resultIndex));
}

IREE::Util::ClosureOpInterface
AsyncWaveOp::cloneReplacementExcludingOperandsAndResults(
    ArrayRef<unsigned> excludedOperandIndices,
    ArrayRef<unsigned> excludedResultIndices, PatternRewriter &rewriter) {
  ...
  auto newOp = rewriter.create<AsyncWaveOp>(
...
CmdExecuteOp::cloneReplacementExcludingOperandsAndResults(
  ...
  auto newOp = rewriter.create<CmdExecuteOp>(getLoc(), await_timepoints(),
