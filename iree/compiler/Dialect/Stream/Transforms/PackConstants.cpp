...
class IndexSet {
 public:
  ...
  Value get(int64_t value) {
    ...
    auto memoizedValue =
        builder.create<mlir::ConstantIndexOp>(loc, value).getResult();
...
static UploadResult buildStagingUpload(
  ...
  auto executeOp = builder.create<IREE::Stream::CmdExecuteOp>(
      constantsOp.getLoc(), /*awaitTimepoints=*/ValueRange{}, capturedResources,
...
static UploadResult buildTryMapConstantResources(
  ...
  auto ok = builder.create<mlir::ConstantIntOp>(constantsOp.getLoc(), 1, 1)
                .getResult();
  ...
  for (auto it : llvm::zip(storageResources, storageBuffers)) {
    ...
    ok = builder.create<mlir::AndOp>(tryMapOp.getLoc(), ok, tryMapOp.did_map());
