 ...
 public:
  ...
  void runOnOperation() override {
    ...
    typeConverter.addConversion([=](TensorType type,
      ...
      resultTypes.push_back(IREE::Stream::ResourceType::get(type.getContext()));
    ...
    typeConverter.addArgumentMaterialization(
        [](OpBuilder &builder, TensorType resultType, ValueRange inputs,
          ...
          // DO NOT SUBMIT need a way to tie dynamic shape dims?
          resultType.dump();
...
#if 0
// DO NOT SUBMIT
    // !stream -> other (tensor/etc)
    typeConverter.addSourceMaterialization([](OpBuilder &builder, Type type,
                                              ValueRange inputs, Location loc) {
      if (auto tensorType = type.dyn_cast<TensorType>()) {
        return buildTensorExportOp(
            loc, inputs.front(), tensorType,
            inputs.take_back(tensorType.getNumDynamicDims()), builder);
      }
      auto cast = builder.create<UnrealizedConversionCastOp>(loc, type, inputs);
      return Optional<Value>(cast.getResult(0));
    });
    // other (tensor/etc) -> !stream
    typeConverter.addTargetMaterialization([](OpBuilder &builder, Type type,
                                              ValueRange inputs, Location loc) {
      if (auto tensorType = inputs.front().getType().dyn_cast<TensorType>()) {
        return buildTensorImportOp(loc, inputs.front(), type, builder);
      }
      auto cast = builder.create<UnrealizedConversionCastOp>(loc, type, inputs);
      return Optional<Value>(cast.getResult(0));
    });
