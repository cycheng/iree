...
static Attribute buildConstantSetAttr(ConstantSet &set, OpBuilder &builder) {
  ...
  for (int64_t site = 0; site < siteCount; ++site) {
    for (int64_t value = 0; value < valueCount; ++value) {
      flattenedAttrs.push_back(set.values[value].second[site]);
    }
  }
  auto tensorType = RankedTensorType::get({siteCount, valueCount}, set.type);
...
//   %0 = constant dense<[[0, 1], [2, 3]]> : tensor<SITExARGxindex>
...
static void insertConstantTableLookup(mlir::FuncOp funcOp,
  ...
  for (auto &set : constantTable.sets) {
    ...
    auto tableTensor = builder.create<mlir::ConstantOp>(
  ...
  for (auto it : llvm::zip(constantTable.sets, tableTensors)) {
    ...
    for (auto operandValues : llvm::enumerate(set.values)) {
      ...
      auto extractOp = builder.create<tensor::ExtractOp>(
          ...
          ValueRange{
              siteId,
              builder.create<mlir::ConstantIndexOp>(arg.getLoc(),
                                                    operandValues.index()),
          });
      arg.replaceAllUsesWith(extractOp);
    }
  }
...
struct MemoizedCmdConstants {
  ...
  Value getIndexForOp(int64_t value, Operation *op) {
    ...
    auto constantValue = OpBuilder(parentOp)
                             .create<mlir::ConstantIndexOp>(op->getLoc(), value)
                             .getResult();
...
std::unique_ptr<OperationPass<ModuleOp>> createSpecializeDispatchesPass() {
  return std::make_unique<SpecializeDispatchesPass>();
}

static PassRegistration<SpecializeDispatchesPass> pass([] {
  return std::make_unique<SpecializeDispatchesPass>();
});
