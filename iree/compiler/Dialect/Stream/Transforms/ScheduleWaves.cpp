...
#define DEBUG_TYPE "iree-stream-schedule-waves"
...
struct PartitionBuilder {
  explicit PartitionBuilder(Block *parentBlock, size_t ordinal,
                            Partition *partition,
    ...
    concurrentOp = parentBuilder.create<IREE::Stream::AsyncWaveOp>(
...
class ScheduleWavesPass : public ScheduleWavesBase<ScheduleWavesPass> {
 public:
  ...
  LogicalResult runOnRegion(IREE::Stream::AsyncExecuteOp parentOp) {
    ...
    // execution region.
    auto waveSet = partitionWaves(configAttr, block);
    ...
    BlockAndValueMapping mapping;
    SmallVector<PartitionBuilder> partitionBuilders;
    ...
    for (auto partition : llvm::enumerate(waveSet.partitions)) {
      ...
      partitionBuilders.push_back(PartitionBuilder(block, partition.index(),
...
std::unique_ptr<OperationPass<>> createScheduleWavesPass() {
  return std::make_unique<ScheduleWavesPass>();
}
