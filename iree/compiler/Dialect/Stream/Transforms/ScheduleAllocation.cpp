...
struct AllocationScope {
  ...
  LivenessIntervalList computeLivenessIntervals() {
    return computeExecutionRegionLivenessIntervals(
        cast<IREE::Stream::AsyncExecuteOp>(rootOp), valueAliases);
  }
  ...
  Value lookupOrCreateIndex(int64_t value) {
    ...
    auto constantValue = OpBuilder(rootOp).createOrFold<ConstantIndexOp>(
        rootOp->getLoc(), value);
  ...
  Value add(Location loc, Value lhs, Value rhs) {
    // TODO(benvanik): memoize - if worth it.
    auto result = OpBuilder(rootOp).createOrFold<mlir::AddIOp>(loc, lhs, rhs);
    return result;
  }

  // Maps |resource| to the storage range defined by |resourceRange|.
  // All aliases of |resource| will also be mapped.
  LogicalResult mapResourceRange(Value resource, ResourceRange resourceRange) {
...
static LogicalResult applyAsyncFillOp(IREE::Stream::AsyncFillOp asyncOp,
  ...
  builder.create<IREE::Stream::CmdFillOp>(
      asyncOp.getLoc(), targetRange.resource, targetRange.resourceSize,
      targetOffset, asyncOp.target_size(), asyncOp.value());
  asyncOp.erase();
  return success();
}
...
static LogicalResult applyAsyncWaveOp(IREE::Stream::AsyncWaveOp asyncOp,
                                      AllocationScope &scope,
                                      OpBuilder &builder) {
  ...
  // They'll be mapped to resources in the parent scope - we must fix this up!
...
static LogicalResult applyAsyncAllocations(Region &region,
  ...
  SmallVector<Operation *> ops = llvm::to_vector<4>(llvm::map_range(
  ...
  for (auto *op : ops) {
    ...
    if (failed(TypeSwitch<Operation *, LogicalResult>(op)
                   .Case<IREE::Stream::ResourceSubviewOp>([&](auto op) {
                     return applyResourceSubviewOp(op, scope, OpBuilder(op));
                   })
                   .Case<IREE::Stream::AsyncAllocaOp>([&](auto op) {
                   ...
                   .Case<IREE::Stream::AsyncSplatOp>([&](auto op) {
                   ...
                   .Case<IREE::Stream::AsyncWaveOp>([&](auto op) {
                     return applyAsyncWaveOp(op, scope, OpBuilder(op));
                   })
                   .Default(failure()))) {
...
static llvm::Optional<TransientAllocation> allocateLocalTransients(
  ...
  AsmState asmState(scope.getRootOp());
  ...
  for (auto valueInterval : scope.computeLivenessIntervals()) {
    ...
    auto allocationSize =
        IREE::Util::SizeAwareTypeInterface::findSizeValue(value, executeOp);
...
struct ConstantReservation {
  ...
  Value resource;
  Value resourceSize;
  // NOTE: may be nullptr if the constant is was not used within the region.
...
static LogicalResult allocateExecutionRegion(
  ...
  llvm::append_range(newAwaitTimepoints, executeOp.await_timepoints());
  ...
  if (transientAllocation.hasValue()) {
    ...
    auto capturedArg = transientAllocation->capturedArg;
    newAwaitTimepoints.push_back(awaitTimepoint);
    ...
    LLVM_DEBUG({
      ...
      llvm::dbgs() << "} captured as ";
      capturedArg.printAsOperand(llvm::dbgs(), asmState);
      llvm::dbgs() << "\n";
    });
  } else {
  ...
  auto newExecuteOp = OpBuilder(executeOp).create<IREE::Stream::CmdExecuteOp>(
      executeOp.getLoc(), newAwaitTimepoints, newOperands, newOperandSizes);
  ...
  newExecuteOp.body().walk<WalkOrder::PreOrder>(
      [&](IREE::Stream::AsyncWaveOp concurrentOp) {
...
class ScheduleAllocationPass
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    for (auto executeOp :
         llvm::to_vector<4>(parentOp.getCallableRegion()
                                ->getOps<IREE::Stream::AsyncExecuteOp>())) {
      if (failed(allocateExecutionRegion(executeOp))) {
