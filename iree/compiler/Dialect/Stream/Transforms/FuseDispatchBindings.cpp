    ...
    for (unsigned oldIdx : binding.value().correlationMap.set_bits()) {
      ...
      for (auto &use : llvm::make_early_inc_range(oldBindingArg.getUses())) {
        if (auto subspanOp =
          ...
          auto sum = builder.createOrFold<mlir::AddIOp>(
...
struct MemoizedCmdZeros {
  ...
  Value getForOp(Operation *op) {
    ...
    auto zero =
        OpBuilder(parentOp).create<mlir::ConstantIndexOp>(op->getLoc(), 0);
...
static void fuseDispatchBindings(
  ...
  LLVM_DEBUG({
    if (bindings.size() == bindingCount) {
      llvm::dbgs() << " (no change, but rebasing)\n";
    }
  });
...
std::unique_ptr<OperationPass<ModuleOp>> createFuseDispatchBindingsPass() {
  return std::make_unique<FuseDispatchBindingsPass>();
}

static PassRegistration<FuseDispatchBindingsPass> pass([] {
  return std::make_unique<FuseDispatchBindingsPass>();
});
