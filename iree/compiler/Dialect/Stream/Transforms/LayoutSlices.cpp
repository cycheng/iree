...
static Value packSlicesWithNoAliasing(
  ...
  for (auto &slice : slices) {
    ...
    offset = IREE::Util::align(
        loc, builder.createOrFold<mlir::AddIOp>(loc, offset, sliceSize),
...
static Value packStaticSlicesGreedily(
  ...
  for (auto &slice : slices) {
    ...
    int64_t staticSize =
        cast<mlir::ConstantIndexOp>(slice.dynamicSize.getDefiningOp())
            .getValue();
    ...
    slice.packedOffset.replaceAllUsesWith(builder.createOrFold<mlir::AddIOp>(
        packOp.getLoc(), baseOffset,
        builder.create<mlir::ConstantIndexOp>(packOp.getLoc(), bestOffset)));
  ...
  return builder.createOrFold<mlir::AddIOp>(
      packOp.getLoc(), baseOffset,
      builder.create<mlir::ConstantIndexOp>(packOp.getLoc(), highwaterMark));
}
  ...
  void runOnOperation() override {
    ...
    parentOp.walk([&](IREE::Stream::ResourcePackOp packOp) {
      ...
      for (auto &slice : allSlices) {
        if (isa_and_nonnull<mlir::ConstantOp>(
      ...
      auto offset =
          packOp.offset()
              ? packOp.offset()
              : builder.createOrFold<mlir::ConstantIndexOp>(packOp.getLoc(), 0);
