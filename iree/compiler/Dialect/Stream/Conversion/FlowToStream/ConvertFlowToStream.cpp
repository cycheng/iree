...
struct ConvertTensorUpdateOp
  ...
  LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<IREE::Stream::TensorUpdateOp>(
        ...
        op.update_dims(), updateSize, rewriter.getIndexArrayAttr({0}),
        /*affinity=*/nullptr);
    return success();
  }
};
...
struct ConvertTensorStoreOp
  ...
  LogicalResult matchAndRewrite(
    ...
    auto newOp = rewriter.replaceOpWithNewOp<IREE::Stream::TensorStoreOp>(
        op, operands.target().getType(), storeTarget, op.target().getType(),
        operands.target_dims(), targetSize, operands.indices(),
        operands.value(), rewriter.getIndexArrayAttr({0}));

    if (operands.target().getType() != stagingType) {
      rewriter.createOrFold<IREE::Stream::AsyncTransferOp>(
          op.getLoc(), operands.target().getType(), newOp.result(), targetSize,
          targetSize,
          /*source_affinity=*/nullptr,
          /*result_affinity=*/nullptr);
    }
...
static SmallVector<Value> makeBindingDynamicDims(
  ...
  auto tieShapeOp = cast<IREE::Flow::DispatchTieShapeOp>(*arg.user_begin());
...
struct ConvertExecutableOp
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto funcOp : moduleOp.getOps<mlir::FuncOp>()) {
      ...
      auto zero = rewriter.create<ConstantIndexOp>(funcOp.getLoc(), 0);
